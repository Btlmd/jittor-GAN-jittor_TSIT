...
def _calc_feat_flatten_mean_std(feat):
    ...
    assert isinstance(feat, jt.float32)
    feat_flatten = feat.view(3, -1)
    mean = feat_flatten.mean(dim=-1, keepdims=True)
    std = feat_flatten.std(dim=-1, keepdims=True)
    return feat_flatten, mean, std


def _mat_sqrt(x):
    U, D, V = jt.linalg.svd(x)
...
def coral(source, target):
    ...
    source_f_cov_eye = \
        mm(source_f_norm, source_f_norm.t()) + jt.eye(3)

    target_f, target_f_mean, target_f_std = _calc_feat_flatten_mean_std(target)
    target_f_norm = (target_f - target_f_mean.expand_as(
        target_f)) / target_f_std.expand_as(target_f)
    target_f_cov_eye = \
        mm(target_f_norm, target_f_norm.t()) + jt.init.eye(3)

    source_f_norm_transfer = mm(
        _mat_sqrt(target_f_cov_eye),
        mm(jt.linalg.inv(_mat_sqrt(source_f_cov_eye)),
                 source_f_norm)
    )
